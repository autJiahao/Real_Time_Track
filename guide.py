...
from utils.general import (
    check_img_size, cv2, xyxy2xywh, non_max_suppression, scale_coords)
            ...
            while True:
                ...
                if self.is_continue:
                    ...
                    pred = non_max_suppression(
                        pred, self.conf, self.iou, classes, agnostic_nms, max_det=max_det)
        ...
        try:
            ...
            while True:
                ...
                if self.is_continue:
                    ...
                    # Apply NMS
                    pred = non_max_suppression(
                        pred, self.conf, self.iou, classes, agnostic_nms)
                            ...
                            for idx, track in enumerate(outputs):
                                ...
                                if len(trajectories[id]) > 1:
                                    for i in range(1, len(trajectories[id])):
                                        cv2.line(
                                            im0, trajectories[id][i - 1], trajectories[id][i], (0, 255, 0), 8)
                                ...
                                for j, (output, conf) in enumerate(zip(outputs, confs)):
                                    ...
                                    annotator.box_label(
                                        bboxes, label, color=color_)

                        prev_trackers = {track[-1]: track[:-1]
...
class Window(QWidget):
    ...
    def __init__(self):
        ...
        self.button_Adaptive = QtWidgets.QSizePolicy(
        ...
        try:
            ...
            self.yolothread.send_img.connect(
                lambda x: self.show_image(x, self.left_label))
            ...
            self.Deepsort_thread.send_img2.connect(
                lambda x: self.show_image2(x, self.right_label))
